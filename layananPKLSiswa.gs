...
function onFormSubmit(e) {
  ...
  try {
    ...
    var noRegister = urutanFormat + "/PKL/Bbws2/" + month + "/" + year;
...
function sendAdminNotification(noReg, sheetUrl) {
  ...
  var subject = "🔔 Permohonan PKL Siswa Baru Masuk!";
  var htmlBody = `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: auto; border: 1px solid #e0e0e0; border-radius: 8px; overflow: hidden;">
      <div style="padding: 20px;">
        <h3 style="color: #1a73e8; margin-top: 0;">Ada Permohonan PKL Siswa Baru</h3>
...
function sendEmailNotification(email, nama, status, noReg, linkSurat, alasan) {
  var subject = "";
  var bodyContent = "";
  var headerColor = "#0B2D72"; 

  if (status === "Diterima") {
    subject = "Layanan PKL siswa - Permohonan Diterima";
    bodyContent = `
    ...
    var noIzin = noReg.replace("PKL", "IZP"); 
    subject = "Layanan PKL Siswa - Permohonan Disetujui";
    bodyContent = `
      <p>Yth. Saudara/i <b>${nama}</b>,</p>
      <p>Dengan hormat kami sampaikan bahwa permohonan PKL Siswa dengan Nomor Register: <b>${noIzin}</b> telah disetujui.</p>
      <p>Persetujuan PKL Siswa disampaikan melalui surat dan dapat diakses pada tautan berikut:</p>
    ...
    subject = "Layanan PKL Siswa - Permohonan Ditolak";
    var alasanText = alasan ? `<p><strong>Alasan:</strong> ${alasan}</p>` : "";
    bodyContent = `
      <p>Yth. Saudara/i <b>${nama}</b>,</p>
      <p>Dengan hormat kami sampaikan bahwa permohonan PKL Siswa dengan Nomor Register: <b>${noReg}</b> belum dapat kami setujui.</p>
